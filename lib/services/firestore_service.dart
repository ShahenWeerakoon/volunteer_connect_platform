...
//     required String message,
//   }) async {
//     await _db.collection('events').doc(eventId).collection('announcements').add({
//       'senderId': senderId,
...
//   }) async {
//     final doc = await _db.collection('events').doc(eventId).collection('announcements').doc(announcementId).get();
//     final reactions = Map<String, dynamic>.from(doc['reactions'] ?? {});
//     if (reactions.containsKey(userId)) reactions.remove(userId);
//     else reactions[userId] = true;
//     await _db.collection('events').doc(eventId).collection('announcements').doc(announcementId).update({'reactions': reactions});
//   }
//
//   Future<void> deleteAnnouncement({required String eventId, required String announcementId}) async {
//     await _db.collection('events').doc(eventId).collection('announcements').doc(announcementId).delete();
//   }
//
//   Future<void> editAnnouncement({required String eventId, required String announcementId, required String message}) async {
//     await _db.collection('events').doc(eventId).collection('announcements').doc(announcementId).update({'message': message});
//   }
//
//   Stream<QuerySnapshot> getAnnouncements(String eventId) {
//     return _db.collection('events').doc(eventId).collection('announcements').orderBy('createdAt', descending: true).snapshots();
//   }
//
//
//
...
class FirestoreService {
  ...
  Future<void> addAnnouncement({
    ...
    required String senderName,
    required String message,
    required String eventName, // Added eventName here
  }) async {
    await _db.collection('events').doc(eventId).collection('announcements').add({
      'eventId': eventId,
      'eventName': eventName, // Store event name for volunteers
      'senderId': senderId,
      'senderName': senderName,
      'message': message,
      'createdAt': FieldValue.serverTimestamp(),
  ...
  Future<void> toggleReaction({
    ...
    final doc = await _db.collection('events').doc(eventId)
        .collection('announcements').doc(announcementId).get();

    final reactions = Map<String, dynamic>.from(doc['reactions'] ?? {});
    if (reactions.containsKey(userId)) reactions.remove(userId);
    else reactions[userId] = true;

    await _db.collection('events').doc(eventId)
  ...
  Future<void> deleteAnnouncement({
    ...
    await _db.collection('events').doc(eventId)
        .collection('announcements').doc(announcementId).delete();
  }
  ...
  Stream<QuerySnapshot> getAnnouncements(String eventId) {
    return _db.collection('events').doc(eventId)
